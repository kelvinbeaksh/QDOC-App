...
import { getDeviceInfo, isPermissionDenied } from "../../../../src/utils/video-helper";
...
jest.mock("../../../../src/utils/video-helper");
...
describe("the useLocalTracks hook", () => {
  beforeEach(() => {
    ...
    jest.spyOn(Video, "createLocalTracks").mockResolvedValue(
      [
        {
          kind: "audio",
          mediaStreamTrack: { getSettings: () => ({ deviceId: "mockAudioTrack" }) } as MediaStreamTrack,
          stop: jest.fn()
        } as unknown as LocalAudioTrack,
        {
          kind: "video",
          mediaStreamTrack: { getSettings: () => ({ deviceId: "mockVideoTrack" }) } as MediaStreamTrack,
          stop: jest.fn()
  ...
  describe("the getAudioAndVideoTracks function", () => {
    ...
    it("should correctly create local audio and video tracks when selected device IDs are available in localStorage",
      async () => {
    ...
    it("should correctly create local audio and video tracks when selected devices " +
      "IDs are available in localStorage, but do not correspond to actual devices", async () => {
    ...
    it("should save the deviceId of the video track to localStorage after it is acquired", async () => {
      ...
      expect(window.localStorage.getItem(SELECTED_VIDEO_INPUT_KEY)).toBe("mockVideoTrack");
    });
    ...
    it("should return an error when there is an error creating a track", async () => {
      jest.spyOn(Video, "createLocalTracks")
        .mockRejectedValueOnce(new Error("testError"));
      const { result } = renderHook(useLocalTracks);

      await act(async () => {
        await expect(result.current.getAudioAndVideoTracks()).rejects.toThrow("testError");
      });
    });
  });
  ...
  describe("the getLocalVideoTrack function", () => {
    it("should create a local video track", async () => {
      ...
      const spy = jest.spyOn(Video, "createLocalVideoTrack").mockResolvedValue({} as LocalVideoTrack);
